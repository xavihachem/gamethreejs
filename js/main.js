...
const test = document.getElementById("final-time");
const test2 = document.getElementById("final-time");
// Constants
...
function updateObstacles(delta) {
  for (let i = 0; i < obstacles.length; i++) {
    ...
    if (obstacle.userData.isMoving) {
      ...
      obstacle.position.x +=
        obstacle.userData.direction * obstacle.userData.speed;

      // Change direction if reached min/max position
      if (
        obstacle.position.x <= obstacle.userData.minX ||
...
function animate() {
  ...
  if (gameStarted && !gameOver) {
    ...
    if (moveForward || moveBackward)
      rabbit.position.z -= direction.z * RABBIT_SPEED * delta;
    if (moveLeft || moveRight)
      rabbit.position.x += direction.x * RABBIT_SPEED * delta;
